...
function openExerciseForm(exercise) {
  ...
  modal.showModal();
  // Handle Cancel click without form validation
  const cancelBtn = actions.querySelector('button[value="cancel"]');
  if (cancelBtn) cancelBtn.addEventListener('click', () => modal.close());

  modalForm.onsubmit = (e) => {
    const v = e.submitter?.value;
    if (v === 'cancel') { modal.close(); return; }
...
// Removed Copy to clipboard from menu per request
